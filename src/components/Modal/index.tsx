...
function Modal() {
  ...
  const MODALS: Modals = useMemo(() => ({
    about: {
      ...
      text: [
        ...
        <p className="modal-text">
          I'm Eric, a full stack web developer specializing in 3D and augmented reality experiences. Thanks for stopping by, and check out this website on a desktop browser to see even more fun details!
        </p>
      </div>
      ],
    },
    ...
    work: {
      ...
      text: [
        <div key={1} className="body">
          <div className="modal-link-over-banner" onClick={() => openModal('moethennessy')}>
            <video
            ...
            <p className="modal-link">
              Moët-Hennessy Concierge Experience
            </p>
          </div>
          <div className="modal-link-over-banner" onClick={() => openModal('kennethcole')}>
            <video
              className="modal-banner"
              src={kennethcole}
              autoPlay={false}
            ...
            />
            <p className="modal-link">
              Kenneth Cole Instagram Filter
            </p>
          </div>
          <div className="modal-link-over-banner" onClick={() => openModal('rosewrapped')}>
            <video
              className="modal-banner"
              src={rosewrapped}
              autoPlay={false}
          ...
          <div className="modal-link-over-banner" onClick={() => openModal('arbor')}>
            <video
              className="modal-banner"
              src={arbor}
              autoPlay={false}
  ...
  const contentToDisplay = useMemo(() => MODALS[modalType], [modalType, MODALS])
  ...
  const handleCloseButton = useCallback((e: MouseEvent<HTMLDivElement>) => {
    if (isDesktop) {
      closeModal()
    } else if (isWorkPanel) {
      openModal('work')
  ...
  return contentToDisplay && (
    ...
    >
      <animated.div className={classNames('modal-contents', { 'has-video': isWorkPanel })} style={contentSprings}>
